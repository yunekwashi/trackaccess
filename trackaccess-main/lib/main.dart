...
class AppState extends ChangeNotifier {
  ...
  bool logVisit(Student student, String library, String detail,
      {int points = 5, bool isDemo = false, DateTime? timestamp}) {
    final effectiveTime = timestamp ?? DateTime.now();
    final dateKey = "${effectiveTime.year}-${effectiveTime.month}-${effectiveTime.day}-${student.uid}";

    if (!isDemo && scannedToday.contains(dateKey)) return false;

    if (!isDemo) scannedToday.add(dateKey);

    visitLogs.add(AttendanceLog(student.name, library, detail, effectiveTime));
  ...
  Future<void> syncWithDatabase() async {
    ...
    final allScans = await ApiService.getAllScans();

    for (var scan in allScans) {
      final uid = scan["uid"] as String;
      final createdAt = DateTime.parse(scan["created_at"] as String);

      final student = authenticateUID(uid);
      if (student != null) {
        // Rebuild the visit log just as it was originally recorded
        logVisit(
          student,
          "College Library", // Default for historical scans
          "Entry",
          timestamp: createdAt,
          isDemo: true, // Use isDemo: true during sync to bypass "scannedToday" daily limit
        );
      }
    }
    notifyListeners();
...
class _StudentModuleState extends State<StudentModule>
  ...
  void _checkForPhysicalScan() async {
    ...
    if (uid != null && uid != _lastScannedUID) {
      _lastScannedUID = uid;
      handleScan(uid, isDemo: false);
    }
  }

  void handleScan(String uid, {bool isDemo = false}) {
...
class _AttendanceAnalyticsState extends State<AttendanceAnalytics> {
  ...
  Future<void> generateReport(List<Student> studentsToExport) async {
    ...
    try {
      final defaultName = 'Library_Report_${DateTime.now().millisecondsSinceEpoch}.csv';

      // Show native Windows Save As dialog via PowerShell (no plugin needed)
      final psScript = '''
Add-Type -AssemblyName System.Windows.Forms;
\$d = New-Object System.Windows.Forms.SaveFileDialog;
\$d.Title = 'Save CSV Report';
      ...
      if (mounted) {
        showDialog(
          ...
          builder: (_) => AlertDialog(
            ...
            content: Text("Successfully saved to:\n$filePath"),
            actions: [
              TextButton(
                  child: const Text("OK"),
                  onPressed: () => Navigator.pop(context)),
